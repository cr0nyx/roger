...
func redirectModeAllowed(info map[int][]byte) bool {
	mode := strings.ToLower(strings.TrimSpace(string(info[MODEOPT])))
	return mode == "" || mode == "classic"
}

func writeClassicInfo(w http.ResponseWriter, info map[int][]byte, statusCode int) {
	w.WriteHeader(statusCode)
	data := blv_encode(info, "optimal", 1024)
...
func isLocalRedirectURL(rawURL string) bool {
	...
	hostIPs, err := net.LookupIP(parsed.Hostname())
	if err != nil {
		return false
	}
	...
	for _, hostIP := range hostIPs {
		...
		for _, localAddr := range localAddrs {
			localIP, _, err := net.ParseCIDR(localAddr.String())
			if err == nil && localIP.Equal(hostIP) {
				return true
			}
		}
	}
...
func relayClassicRequest(w http.ResponseWriter, r *http.Request, info map[int][]byte, head, tail []byte) bool {
	...
	forceRedirect := strings.EqualFold(string(info[FORCEREDIRECT]), "TRUE")
	if !forceRedirect && isLocalRedirectURL(redirectURL) {
		return false
	}
	delete(info, REDIRECTURL)
	delete(info, FORCEREDIRECT)
	encoded := base64encode(blv_encode(info, "optimal", 1024))
	body := make([]byte, 0, len(head)+len(encoded)+len(tail))
...
func deleteSession(mark string) {
	...
	delete(settings, mark)
	lock.Unlock()
}

func closeSession(mark string, sess *session) {
	if sess != nil {
		sess.Close()
	}
	deleteSession(mark)
...
func handleFullDuplex(w http.ResponseWriter, r *http.Request, reader io.Reader, first map[int][]byte) {
	if len(first[REDIRECTURL]) > 0 {
		probeSession := &session{settings: defaultSettings()}
		_ = writeStreamFrame(w, probeSession, map[int][]byte{
			STATUS: []byte("FAIL"),
...
func roger(w http.ResponseWriter, r *http.Request) {
	...
	if USE_REQUEST_TEMPLATE == 1 && len(data) > 0 {
		...
		requestDataTail = append([]byte(nil), data[len(data)-END_INDEX:]...)
	...
	if err == nil && len(out) != 0 {
		...
		case "BIND":
			...
			bindSess := &session{
				...
				tcp:          true,
				udpin:        make(map[uint32]*udpReassembly),
				settings:     sessionSettingsFromRequest(mark, info),
				lastActivity: time.Now(),
			}
			setSession(mark, bindSess)

			go func() {
				defer l.Close()
				for {
					if bindSess.isClosed() {
						return
					}
					...
					lock.Lock()
					current := sessions[mark]
					if current != bindSess || bindSess.closed {
						lock.Unlock()
						conn.Close()
						return
					}
					sess := newSession(conn, true, bindSess.settings)
